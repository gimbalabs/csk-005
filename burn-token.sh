...
NETWORK_MAGIC=3
SOURCE_ADDRESS=$(cat ../../csk005/base.addr)
DESTINATION_ADDRESS=$(cat ../../csk005/base.addr)
PAYMENT_SKEY=../../csk005/payment.skey
TOKEN_NAME=SPAM9511token
...
sed -i "s|$(cat /tmp/balance.${TOKEN_POLICY})|$(( $(cat /tmp/balance.${TOKEN_POLICY}) - ${BURN_AMOUNT} ))|g" /tmp/balance.${TOKEN_POLICY}
...
cardano-cli transaction build-raw \
            ...
            --fee ${ACTUAL_MIN_FEE} \
	    ${IN_UTXO} \
	    --tx-out="${SOURCE_ADDRESS}+$(( $(cat /tmp/balance.lovelace) - ${ACTUAL_MIN_FEE})) lovelace${OTHER_COINS}" \
	    --mint="-${BURN_AMOUNT} ${TOKEN_POLICY}" \
